...
variable "cluster_permissions_boundary_arn" {
  type        = string
  description = "ARN da permissions boundary dedicada a role do cluster EKS de homologacao."

  validation {
    condition     = can(regex("^arn:aws:iam::[0-9]{12}:policy/oficina-mecanica-homologacao-eks-cluster-boundary$", var.cluster_permissions_boundary_arn))
    error_message = "cluster_permissions_boundary_arn deve ser a boundary do cluster de homologacao."
  }
}

variable "node_permissions_boundary_arn" {
  type        = string
  description = "ARN da permissions boundary dedicada a role dos nodes EKS de homologacao."

  validation {
    condition     = can(regex("^arn:aws:iam::[0-9]{12}:policy/oficina-mecanica-homologacao-eks-node-boundary$", var.node_permissions_boundary_arn))
    error_message = "node_permissions_boundary_arn deve ser a boundary dos nodes de homologacao."
  }
}

variable "deployer_permissions_boundary_arn" {
  type        = string
  description = "ARN da permissions boundary dedicada a role de deploy de homologacao."

  validation {
    condition     = can(regex("^arn:aws:iam::[0-9]{12}:policy/oficina-mecanica-homologacao-api-deployer-boundary$", var.deployer_permissions_boundary_arn))
    error_message = "deployer_permissions_boundary_arn deve ser a boundary do deployer de homologacao."
  }
}

variable "vpc_cidr" {
  type        = string
  description = "CIDR principal da VPC de homologacao."
}
...
variable "single_nat_gateway" {
  type        = bool
  description = "Quando true, compartilha um NAT Gateway entre as zonas."
}
...
variable "tags" {
  type        = map(string)
  description = "Tags adicionais dos recursos de homologacao."
  default     = {}
}
