...
override_resource {
  ...
  override_during = plan
  values = {
    name = "oficina-mecanica-producao"
    certificate_authority = [{
      data = "dGVzdC1jYQ=="
    }]
    identity = [{
      oidc = [{
        issuer = "https://oidc.eks.us-east-1.amazonaws.com/id/PRODUCAO"
      }]
    }]
    vpc_config = {
      cluster_security_group_id = "sg-cluster-producao"
...
override_resource {
  ...
  override_during = plan
  values = {
    arn = "arn:aws:iam::123456789012:role/oficina-mecanica-producao-api-deployer"
  }
}
...
variables {
  ...
  cluster_permissions_boundary_arn     = "arn:aws:iam::123456789012:policy/oficina-mecanica-producao-eks-cluster-boundary"
  node_permissions_boundary_arn        = "arn:aws:iam::123456789012:policy/oficina-mecanica-producao-eks-node-boundary"
  deployer_permissions_boundary_arn    = "arn:aws:iam::123456789012:policy/oficina-mecanica-producao-api-deployer-boundary"
...
run "producao_root_exposes_safe_wrapper_contract" {
  command = plan

  assert {
    condition     = output.cluster_name == "oficina-mecanica-producao"
    error_message = "The environment root must expose cluster_name for the guarded output wrapper."
  }

  assert {
    condition     = output.deployer_role_arn == "arn:aws:iam::123456789012:role/oficina-mecanica-producao-api-deployer"
