...
run "network_has_isolated_database_tier" {
  ...
  assert {
    condition     = aws_route.public.destination_cidr_block == "0.0.0.0/0" && length(aws_route.private) == 2
    error_message = "Only public and private tiers may have managed default routes; the database tier must remain isolated."
  }
  ...
  assert {
    condition = alltrue([
      for index, route in aws_route.private :
      route.destination_cidr_block == "0.0.0.0/0" && route.nat_gateway_id == aws_nat_gateway.this[var.single_nat_gateway ? 0 : index].id
    ])
